...
df <- read.table("mergedCNA.tsv", header = T , sep = "\t", stringsAsFactors = F)
...
dfm$variable <- factor(dfm$variable, levels=c("ILL", "ONT", "PB", "TENX", "BNG"))
...
ggsave(plot = p, filename = "cna_plot.pdf", device="pdf", width=14, height = 8)
